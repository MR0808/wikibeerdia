...
interface SubStylesTableContextProps {
    featureFlags: FeatureFlagValue[];
    setFeatureFlags: React.Dispatch<React.SetStateAction<FeatureFlagValue[]>>;
}

const SubStylesTableContext = createContext<SubStylesTableContextProps>({
    featureFlags: [],
    setFeatureFlags: () => {}
});

export const useSubStylesTable = () => {
    const context = useContext(SubStylesTableContext);
    if (!context) {
        throw new Error(
...
export const SubStylesTableProvider = ({
    children
}: React.PropsWithChildren) => {
    const [featureFlags, setFeatureFlags] = useState<FeatureFlagValue[]>([]);

    return (
        <SubStylesTableContext.Provider
            value={{
                featureFlags,
                setFeatureFlags
            }}
